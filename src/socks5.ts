...
const REPLY_CODES: Record<number, string> = {
...
function replyText(code: number): string {
 ...
 */
export function openSocks5Tunnel(
  targetHost: string,
  targetPort: number,
  opts: Socks5Options,
): Promise<net.Socket> {
    ...
    function onData(chunk: Buffer): void {
      ...
      try {
        if (stage === 'greeting') {
          ...
          if (ver !== 0x05) throw new Error(`SOCKS5 proxy ${opts.host}:${opts.port} replied with unsupported version ${ver}`)
          if (method === 0xff) throw new Error(`SOCKS5 proxy ${opts.host}:${opts.port} requires authentication, which this plugin does not support`)
          if (method !== 0x00) throw new Error(`SOCKS5 proxy ${opts.host}:${opts.port} selected unsupported auth method 0x${method.toString(16)}`)
          buffer = buffer.subarray(2)
          stage = 'connect'
          // CONNECT 请求：CMD=1, RSV=0, ATYP=3(域名)
          const hostBuf = Buffer.from(targetHost, 'utf8')
          if (hostBuf.length > 255) throw new Error(`target hostname too long: ${targetHost}`)
        ...
        const rep = buffer[1]
        if (ver !== 0x05) throw new Error(`SOCKS5 proxy ${opts.host}:${opts.port} replied with unsupported version ${ver}`)
        if (rep !== 0x00) throw new Error(`SOCKS5 proxy ${opts.host}:${opts.port} refused connection to ${targetHost}:${targetPort}: ${replyText(rep)}`)
        // 跳过 BND.ADDR（按 ATYP 变长）与 BND.PORT(2)
        const atyp = buffer[3]
        let addrLen: number
        if (atyp === 0x01) addrLen = 4
        else if (atyp === 0x03) addrLen = buffer[4]
        else if (atyp === 0x04) addrLen = 16
        else throw new Error(`SOCKS5 proxy ${opts.host}:${opts.port} returned unknown address type 0x${atyp.toString(16)}`)
