...
export function decoderForCharset(charset: string | undefined): InstanceType<typeof TextDecoder> {
...
export function parseStatusLine(line: string): number {
  ...
  if (!match) throw new WebError(`malformed HTTP status line: "${line}"`, 'WEB_PROVIDER_ERROR')
